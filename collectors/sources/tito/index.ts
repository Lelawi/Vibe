...
export async function run() {
  ...
  for (const url of urls) {
    try {
      ...
      if (!res.ok) { console.warn('[tito] fetch failed', res.status); continue; }
    ...
    } catch (err) { console.warn('[tito] error', err); }
  }

  if (collected.length) { console.log('[tito] upserting', collected.length); const { error } = await supabase.from('events').upsert(collected, { onConflict: 'source_id' }); if (error) console.error('[tito] upsert error', error); }
}
